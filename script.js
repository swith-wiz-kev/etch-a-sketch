    ...
    case "whiten":
      const rgb2hex = (rgb) =>
        `${rgb
          ...
          .join("")}`;
      const hexValue = rgb2hex(
        window
          .getComputedStyle(event.target)
          .getPropertyValue("background-color")
      );
      const hexValue1 = Math.min(
        Number(`0x${hexValue.slice(0, 2)}`) + 0x1a,
        0xff
      );
      const hexValue2 = Math.min(
        Number(`0x${hexValue.slice(2, 4)}`) + 0x1a,
        0xff
      );
      const hexValue3 = Math.min(
        Number(`0x${hexValue.slice(4, 6)}`) + 0x1a,
        0xff
      );
      const stringHex =
        "#" +
        hexValue1.toString(16) +
        hexValue2.toString(16) +
        hexValue3.toString(16);
...
function newGrid() {
  ...
  while (!(sideLength >= 2 && sideLength <= 100)) {
    sideLength = prompt(
      "Please enter number of squares in one side of new grid. (2-100)"
    );
